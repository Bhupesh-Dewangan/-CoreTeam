...
export const inngest = new Inngest({ id: "Core-Team" });
...
const autoCheckOut = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        if (attendance && !attendance.checkOut) {
            ...
            await step.sleepUntil("wait-for-the-1-hour", new Date(new
...
// Send Email to admin, if admin doesn't respond to leave application within 24 hours
...
const attendanceReminderCron = inngest.createFunction(
    ...
    async ({ step }) => {
        ...
        const today = await step.run("get-today-date", () => {
            ...
            return {
                startUTC: startUTC.toISOString(), endUTC:
                    endUTC.toISOString()
            }
        ...
        const checkedInIds = await step.run("get-checked-in-ids", async () => {
            const attendances = await Attendance.find({
                date: {
                    $gte: new Date(today.startUTC), $lte: new
                        Date(today.endUTC)
                },
            }).lean();
        ...
        return { totalActive: activeEmployees.length, onLeave: onLeaveIds.length, checkedIn: checkedInIds.length, absent: absentEmployees }
    }
)
